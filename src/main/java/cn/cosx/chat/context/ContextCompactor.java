...
public class ContextCompactor {
    ...
    /**
     * auto_compact：将旧消息（除 SystemMessage 和当前问题）交给 LLM 生成结构化摘要，然后替换。
     */
    private void autoCompact(List<Message> messages, String currentQuestion) {
        ...
        // 2. 所有非 SystemMessage 的消息进入摘要，当前轮问题单独保留在末尾
        List<Message> oldMessages = new ArrayList<>(
                messages.subList(systemMsgCount, messages.size()));
        Message currentQuestionMessage = removeCurrentQuestionMessage(oldMessages, currentQuestion);
    ...
    private Message removeCurrentQuestionMessage(List<Message> messages, String currentQuestion) {
        if (messages == null || messages.isEmpty() || currentQuestion == null || currentQuestion.isBlank()) {
            return null;
        }

        String wrappedCurrentQuestion = "<question>" + currentQuestion + "</question>";
        for (int i = messages.size() - 1; i >= 0; i--) {
            if (!(messages.get(i) instanceof UserMessage userMessage)) {
                continue;
            }
            String text = userMessage.getText();
            if (Objects.equals(text, currentQuestion)
                    || Objects.equals(text, wrappedCurrentQuestion)
                    || (text != null && text.contains(currentQuestion))) {
                messages.remove(i);
                return userMessage;
            }
        }
    ...
    /**
     * 提取消息的文本内容（未开启思考模式，按普通消息内容处理，ToolCall 保留名称和参数）。
     */
